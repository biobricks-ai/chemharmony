...
pcols <- c("toxval_type","toxval_type_original","toxval_subtype",
  ...
  "study_type","study_duration_class","study_duration_value","species_common","species_supercategory",
  "habitat","human_eco","strain","sex","generation","lifestage","exposure_route","exposure_method",
  "exposure_form","media","media_original","critical_effect","critical_effect_original")

tval2 = tval |> group_by(!!!syms(pcols)) |> mutate(pid = UUIDgenerate()) |> ungroup()
